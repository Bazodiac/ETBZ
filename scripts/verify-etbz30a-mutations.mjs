...
const MUTANTS = [
  // --- bindings ---------------------------------------------------------------------
  ['BIND: an unverified brief is accepted', GRAPH, "if (context.brief.structuralHash !== rederived.brief.structuralHash) {", "if (context.brief.structuralHash === '') {", [T.unit]],
  ...
  ['CLAIM: the claim validator is not composed', GRAPH, "    validateInterpretiveClaim(claim, validation);\n", "", [T.negative]],
  ...
  ['GRAPH: a theme the brief does not contain', GRAPH, "      if (!themeIds.has(themeRef)) {", "      if (!themeIds.has(themeRef) && themeRef === '') {", [T.negative]],
  ['GRAPH: the same meaning twice becomes two claims', GRAPH, "    if (twin !== undefined) {", "    if (twin !== undefined && drafts.length < 0) {", [T.negative]],
  ...
  // --- integrity and PD-5 ------------------------------------------------------------------
  ['INTACT: an edited graph passes', GRAPH, "  if (structuralHash(graph) !== structuralHash(rebuilt)) {", "  if (structuralHash(graph) === '') {", [T.unit]],
...
function run(tests) {
  return spawnSync('npx', ['vitest', 'run', ...tests], { encoding: 'utf8' }).status;
}
